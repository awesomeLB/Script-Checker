...
class GraphFlows(nn.Module):
    ...
    def save(self, path_1, path_2):
        torch.save(self.state_dict(), path_1)
        torch.save(self.base_logit_probs, path_2)

    def load(self, path_1, path_2):
        self.load_state_dict(torch.load(path_1))
        self.base_logit_probs = torch.load(path_2)
...
class DiscreteCouplingFlow(AbstractInvertibleLayer):
    ...
    def forward_transform(self, input, auxiliary_input=None):
        ...
        # __v2 __convert input to one_hot without losing gradient___
        input_by  = 1. - input_b
